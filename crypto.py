...
def decrypt_update(encrypted_data, round_key, associated_data):
    """
    SECURITY: `associated_data` is a required positional argument, and it is
    the ONLY source of truth for the AAD used to verify this ciphertext.

    A "two-path" version of this function briefly existed that additionally
    fell back to encrypted_data["associated_data"] when associated_data=None
    was passed, on the reasoning that a server-side caller could safely
    pre-load that dict key after independently recomputing it. That
    reasoning does not hold as a library-level invariant: `encrypted_data`
    is, at multiple call sites in this codebase (and in the real production
    path once client FitRes payloads are involved), the same dict/object
    that arrived from the network, so a fallback keyed on that dict is only
    ...
    use no AAD. Nothing is ever read from `encrypted_data` for this purpose.
    """
    aesgcm = AESGCM(bytes(round_key))
    aad = _format_aad(associated_data)
