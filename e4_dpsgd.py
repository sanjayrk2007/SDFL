...
class DPSGDHospitalClient(HospitalClient):
    def __init__(self, hospital_id, trainloader, valloader, local_epochs=3, mu=0.0, max_grad_norm=1.0, noise_multiplier=1.0):
        ...
        convert_bn_to_gn(self.model, num_groups=4)
    ...
    def fit(self, parameters, config):
        ...
        convert_bn_to_gn(global_model, num_groups=4)
...
def main():
    ...
    convert_bn_to_gn(initial_model, num_groups=4)
    ...
    for res in sweep_results:
        print(f"C={res['C']}, sigma={res['sigma']}, val_dice={res['val_dice']:.4f}, val_iou={res['val_iou']:.4f}, epsilon={res['epsilon']:.4f}")
    if best_ndarrays is not None:
        best_model = ResUNetPlusPlus().to(DEVICE)
        convert_bn_to_gn(best_model, num_groups=4)
        set_parameters(best_model, best_ndarrays)
        torch.save(best_model.state_dict(), "checkpoints/e4_best.pth")
    print("Best Configuration:")
    print(f"C={best_params[0]}, sigma={best_params[1]}, val_dice={best_params[2]:.4f}, val_iou={best_params[3]:.4f}, epsilon={best_params[4]:.4f}")
